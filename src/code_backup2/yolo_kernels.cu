...
extern "C" void demo_yolo(char *cfgfile, char *weightfile, float thresh, char *videofile)
{
    demo_thresh = thresh;
    printf("YOLO demo\n");
...
/*
    //output video
    //char outputVideoName[256];
    //sprintf(outputVideoName, "%s_output.avi", videofile); 
    ...
    //outputVideo.open(outputVideoName, -1, videoCap.get(CV_CAP_PROP_FPS), S, true);
...
*/
    ...
    while(1){
        ...
        //Mat outputMat= image_to_Mat(disp, w, h, depth, c);
/*
        IplImage* outputIpl= image_to_Ipl(disp, w, h, depth, c, step);
        cv::Mat outputMat = cv::cvarrToMat(outputIpl, true);
        cvReleaseImage(&outputIpl);
        outputVideo << outputMat;
        outputMat.release();
 */
